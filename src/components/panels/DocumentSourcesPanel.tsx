...
import { saveSessionData, getSessionData, updateSessionDocuments } from '@/lib/sessionStorage';

type UsageCounts = { uploads: number; queries: number; total: number };
type FreemiumError = { limitExceeded: boolean; currentUsage?: UsageCounts };
...
function isUsageCounts(x: unknown): x is UsageCounts {
  if (!x || typeof x !== 'object') return false;
  const r = x as Record<string, unknown>;
  return (
    typeof r.uploads === 'number' &&
    typeof r.queries === 'number' &&
    typeof r.total === 'number'
  );
}

function isFreemiumError(x: unknown): x is FreemiumError {
  if (!x || typeof x !== 'object') return false;
  const r = x as Record<string, unknown>;
  const hasLimitExceeded = typeof r.limitExceeded === 'boolean';
  const hasValidCurrent =
    r.currentUsage === undefined || isUsageCounts(r.currentUsage);
  return hasLimitExceeded && hasValidCurrent;
}
...
export default function DocumentSourcesPanel() {
    ...
    const loadSessionData = async () => {
      try {
        ...
        if (sessionData && sessionData.documents) {
          console.log('Loading documents from session:', sessionData.documents.length);
        ...
        if (file.type === 'text/plain' || file.type === 'application/pdf') {
          ...
          try {
            ...
            if (response.ok) {
              ...
              if (isFreemiumError(data) && data.limitExceeded) {
                // Prefer server-provided counts if present; otherwise use local usage
                showUpgradeModal('upload', data.currentUsage ?? usage);
                return;
              }
  ...
  const handleTextSubmit = useCallback(
      ...
      try {
        ...
        if (response.ok) {
          ...
          const textBody = await response
            ...
            .text()
            .catch(() => '');
          const data = parseJsonSafe(textBody);
          if (isFreemiumError(data) && data.limitExceeded) {
            showUpgradeModal('upload', data.currentUsage ?? usage);
            return;
          }
          console.error('Text upload error:', response.status, textBody);
    ...
    [updateUsage, showUpgradeModal, usage],
  );
  ...
  const handleUrlSubmit = useCallback(
    async (url: string, type: 'youtube' | 'website') => {
      ...
      const tempId = `temp_url_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      const newDoc: Document = {
        id: tempId,
      ...
      // Show loading toast
      const loadingToast = toast.loading(`Scraping ${type === 'youtube' ? 'YouTube video' : 'website'} content...`);

      try {
        console.log('Starting URL scraping for:', url, 'Type:', type);
        ...
        // Add timeout to prevent hanging requests
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 120000); // 2 minute timeout for scraping
        ...
        if (response.ok) {
          ...
          // Dismiss loading toast and show success
          toast.dismiss(loadingToast);
          toast.success(`${type === 'youtube' ? 'YouTube video' : 'Website'} content scraped successfully!`, {
            duration: 3000,
          });
        } else {
          const text = await response
            ...
            .text()
            .catch(() => '');
          console.error('Scrape failed with status:', response.status, 'Response:', text);
          const data = parseJsonSafe(text);
          if (isFreemiumError(data) && data.limitExceeded) {
            // Prefer server-provided counts if present; otherwise use local usage
            showUpgradeModal('upload', data.currentUsage ?? usage);
          ...
          throw new Error(`Scraping failed with status ${response.status}: ${response.statusText}`);
        }
      } catch (error) {
        console.error('URL scraping error:', error);
        
        // Show user-friendly error message
        let errorMessage = `Failed to scrape ${type === 'youtube' ? 'YouTube video' : 'website'}`;
        if (error instanceof Error) {
          if (error.name === 'AbortError') {
            errorMessage = 'Scraping timeout - content took too long to process';
            console.error('Scraping timed out after 2 minutes');
          } else if (error.message.includes('fetch')) {
            errorMessage = 'Network error - check if server is running';
            console.error('Network error - check if server is running');
          } else {
  ...
  const _getStatusColor = (status: Document['status']) => {
    switch (status) {
      case 'uploading':
