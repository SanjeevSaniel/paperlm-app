...
import { getSessionId, NotebookNote } from '@/lib/sessionStorage';
...
// API Note type for dialog compatibility
interface APINote {
  id: string;
  title: string;
  content: string;
...
export default function SmartNotebookPanel() {
  ...
  const [processedDocuments, setProcessedDocuments] = useState<Set<string>>(
    new Set(),
  );
    ...
    (documentContent: string, documentName: string, sourceUrl?: string) => {
      ...
      if (geminiSummaryMatch && geminiSummaryMatch[1]) {
        ...
        if (sourceUrl && sourceUrl.includes('youtube')) {
          const originalTranscriptMatch = documentContent.match(
            /# 📄 Original Transcript\s*([\s\S]*)/,
          );
          const originalTranscript = originalTranscriptMatch
            ? originalTranscriptMatch[1].trim()
            : '';
      ...
      if (aiSummaryMatch && aiSummaryMatch[1]) {
        ...
        if (sourceUrl) {
          ...
          if (contentPreview) {
            ...
            enhancedContent += `> ${contentPreview}${
  ...
  const generateSmartSummary = useCallback((doc: Document) => {
    ...
    // const fullDocument = null; // Placeholder for now - will be used when API integration is complete
    const rawContent: string = ''; // Will be updated when API integration is complete
    ...
    if (type === 'video/youtube') {
      ...
      if (rawContent) {
        ...
        const transcriptLines = rawContent
          .split('\n')
      ...
      if (rawContent) {
        ...
        const paragraphs = rawContent
          .split('\n\n')
          .filter((p: string) => p.trim() && p.length > 50);
        if (paragraphs.length > 1) {
          content += `### 📋 Content Structure\n`;
          content += `- **Total Sections**: ${paragraphs.length}\n`;
          content += `- **Average Section Length**: ${Math.round(
            rawContent.length / paragraphs.length,
          )} characters\n`;
  ...
  const generateAINotebookCards = useCallback(
    async (documents: Document[]) => {
      console.log(
        '🎯 generateAINotebookCards called with',
        documents.length,
        'documents',
      );
      try {
        for (const doc of documents) {
          ...
          const aiSummary = extractAISummary(
            '', // Will be updated when API integration is complete
            doc.name,
            doc.sourceUrl,
          );
    ...
    [generateSmartSummary, extractAISummary, user?.id],
  );

  // Load notes from NeonDB API on mount and when refresh is triggered
  useEffect(() => {
    const loadNotesFromAPI = async () => {
      try {
        ...
        if (notesResponse.ok) {
          ...
          const apiNotes: Note[] = notesData.notes.map((note: APINote) => {
            // Determine note type based on title and content
            let noteType: 'summary' | 'insight' | 'quote' = 'summary';
            if (
              note.title.includes('Citation:') ||
              note.content.includes('Citation Content')
            ) {
              noteType = 'quote';
            } else if (
              note.title.includes('Insight') ||
          ...
          const existingDocIds = new Set(
            apiNotes
              .map((note) => note.sourceDocumentId)
              .filter((id) => id) as string[],
          );
          setProcessedDocuments(existingDocIds);
        } else {
          console.error('Failed to load notes:', notesResponse.statusText);
          setNotes([]);
        }
      } catch (error) {
  ...
  // Save notes to NeonDB API whenever they change (debounced)
  useEffect(() => {
    ...
    const saveNotesToAPI = async () => {
      try {
        ...
        // Save each note that has been modified
        for (const note of notes) {
          // Skip if note is a processing placeholder
          if (note.id.startsWith('processing-')) continue;
    ...
    const timeoutId = setTimeout(saveNotesToAPI, 1000);
  ...
  useEffect(() => {
    console.log('🔍 Auto-generation check:', {
      ...
      documentCount,
      isInitialized,
      processedDocsCount: processedDocuments.size,
    });
    if (hasDocuments && documentCount > 0 && isInitialized) {
      ...
      console.log(
        '🏷️ Previously processed docs:',
        Array.from(processedDocuments),
      );
      ...
      const newDocuments = documents.filter((doc) => {
        ...
        const hasNote = existingNotes.some(
          (note) => note.sourceDocumentId === doc.id,
        );
        const alreadyProcessed = processedDocuments.has(doc.id);

        console.log(
          `📋 Doc ${doc.name}: status=${doc.status}, hasNote=${hasNote}, alreadyProcessed=${alreadyProcessed}`,
        );
      ...
      if (newDocuments.length > 0) {
        console.log(
          '🚀 Starting auto-generation for:',
          newDocuments.map((d) => d.name),
        );
  ...
  const saveEdit = async () => {
    if (editingId) {
      ...
      try {
          ...
          if (noteToUpdate) {
            await fetch('/api/notes', {
              ...
              body: JSON.stringify({
                ...
                sessionId: sessionId,
                metadata: {
                  sourceDocumentId: noteToUpdate.sourceDocumentId,
                  sourceUrl: noteToUpdate.sourceUrl,
        ...
        console.error('Failed to save edited note to API:', error);
      }
  ...
  const handleNewNoteSave = async (newNote: APINote) => {
    const noteWithNew: Note = {
      ...newNote,
      isNew: true,
      userId: user?.id || 'anonymous',
      timestamp: Date.now(),
      sourceDocumentId: newNote.sourceDocumentId,
    ...
    try {
      ...
      if (sessionId) {
        await fetch('/api/notes', {
          ...
          body: JSON.stringify({
            id: noteWithNew.id,
            title: noteWithNew.title,
            content: noteWithNew.content,
            sessionId: sessionId,
            metadata: {
              sourceDocumentId: noteWithNew.sourceDocumentId,
              sourceUrl: noteWithNew.sourceUrl,
              tags: noteWithNew.tags || [],
              type: noteWithNew.type,
            },
          }),
        });
      }
  ...
  const handleSaveFromDialog = async (updatedNote: APINote) => {
    const convertedNote: Note = {
      ...updatedNote,
      userId: user?.id || 'anonymous',
      timestamp: Date.now(),
      sourceDocumentId: updatedNote.sourceDocumentId,
      sourceUrl: updatedNote.sourceUrl,
      tags: updatedNote.tags || [],
    };
    setNotes((prev) =>
      prev.map((note) => (note.id === updatedNote.id ? convertedNote : note)),
    );
      ...
      if (sessionId) {
        await fetch('/api/notes', {
          ...
          body: JSON.stringify({
            ...
            sessionId: sessionId,
            metadata: {
              sourceDocumentId: convertedNote.sourceDocumentId,
              sourceUrl: convertedNote.sourceUrl,
              tags: convertedNote.tags || [],
              type: convertedNote.type,
            },
          }),
        });
      }
    } catch (error) {
      console.error('Failed to save updated note to API:', error);
    }
  };
  ...
  const handleDeleteNote = async (noteId: string) => {
    ...
    try {
      const sessionId = getSessionId();
      if (sessionId) {
        await fetch(`/api/notes?noteId=${noteId}&sessionId=${sessionId}`, {
          method: 'DELETE',
        });
      }
    } catch (error) {
  ...
  return (
    <div className='h-full flex flex-col'>
      ...
      <div
        ...
        {!hasDocuments && notes.length === 0 && !isLoading && (
          <motion.div
